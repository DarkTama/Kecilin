...
- ✅ **Performance honesty** — `veryfast` preset on web, a banner linking to Releases. **v0.6.1 pins the single-thread core**: `@ffmpeg/core-mt` 0.12.x wedges on repeat execs with WORKERFS mounts (probe hangs; thumbnails/converts die after the first run — reproduced locally with COOP/COEP preview headers, matching the field failure on a 169 MB file). Revisit mt when upstream stabilizes; the coi-serviceworker machinery went with it. Multi-GB files remain out of scope (wasm memory cap).
